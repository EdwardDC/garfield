...
export default {
  ...
  methods: {
    getKeyOfRow (row, index) {
      ...
      if (rowKey) {
        return _.getRowIdentity(row, rowKey)
      }
      return index
    },
    ...
    handleCellMouseEnter (event, row) {
      ...
      const cell = _.getCell(event)

      if (cell) {
        const column = _.getColumnByCell(table, cell)
      ...
      if (_.hasClass(cellChild, 'el-tooltip') && cellChild.scrollWidth > cellChild.offsetWidth) {
    ...
    handleEvent (event, row, name) {
      ...
      let column
      if (cell) {
        column = _.getColumnByCell(table, cell)
        if (column) {
